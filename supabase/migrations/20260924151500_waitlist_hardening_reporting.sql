-- Keep owner verification rows out of real demand metrics and count UES-owned storage as confirmed.
  ...
  select count(*)::int as total,
         ...
         count(*) filter (
           where event_name = 'waitlist_signup'
             and coalesce(source,'') <> 'owner_test'
             and (
               lower(coalesce(metadata->>'providerConfirmed','false')) = 'true'
               or lower(coalesce(metadata->>'ownedStorage','false')) = 'true'
             )
