...
export function partnershipScore(opportunity, { focusTerms = DEFAULT_FOCUS_TERMS, now = new Date() } = {}) {
  validatePartnershipOpportunity(opportunity, now);
  ...
  if (risks.has("approval-not-guaranteed") || risks.has("acceptance-not-guaranteed") || risks.has("selection-not-guaranteed")) score -= 2;
  return score;
}
...
export function rankPartnershipOpportunities(opportunities, options = {}) {
  return (opportunities ?? [])
    ...
    .map((opportunity) => ({ opportunity: validatePartnershipOpportunity(opportunity, options.now), score: partnershipScore(opportunity, options) }))
...
export function buildPartnershipCandidate(opportunities, { date = new Date(), focusTerms = DEFAULT_FOCUS_TERMS } = {}) {
  const ranked = rankPartnershipOpportunities(opportunities, { focusTerms, now: date });
