...
export function partnershipScore(opportunity, { focusTerms = ["collectibles", "youtube", "shorts"] } = {}) {
...
export function buildPartnershipCandidate(opportunities, { date = new Date(), focusTerms = ["collectibles", "youtube", "shorts"] } = {}) {
  const ranked = rankPartnershipOpportunities(opportunities, { focusTerms });
  if (!ranked.length) throw new Error("No verified partnership opportunities are available");
  ...
  const candidate = {
    ...
    shortlist: ranked.slice(0, 4).map(({ opportunity, score }) => ({
      id: opportunity.id,
    ...
    finder: {
      job: "Rank verified opportunities by sponsorship value, collectible fit, accessibility, and policy risk.",
    ...
    outreachBrief: {
      ...
      positioning: "BlindBoxAI creates evidence-backed collectible Buy-or-Pass content and routes interested viewers to transparent research and shopping paths.",
      ...
      proofNeededBeforeSending: ["current channel metrics", "audience geography", "recent relevant content performance", "verified eligibility", "brand-specific fit"],
    },
