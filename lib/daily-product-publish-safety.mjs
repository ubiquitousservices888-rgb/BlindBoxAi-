...
import { assertAffiliateEligibilityRecord } from "./market-eligibility.mjs";
...
export function hardenCandidateForPublishing(candidate) {
  ...
  assertAffiliateEligibilityRecord(next.affiliateEligibility);
...
export function assertCandidateCtas(candidate) {
  ...
  assertAffiliateEligibilityRecord(candidate.affiliateEligibility);
...
export async function discoverBufferChannels({ token, organizationId, services, fetchImpl = fetch }) {
  ...
  if (!requestedServices.size) throw new Error("At least one Buffer service is required");
  ...
  return (data?.channels ?? [])
    .filter((channel) => !channel.isLocked
      ...
      && requestedServices.has(channel.service))
    .map((channel) => ({
      ...channel,
      organizationId: organization.id,
...
export async function discoverScopedBufferChannels({ token, organizationId, fetchImpl = fetch }) {
  return discoverBufferChannels({
    token,
    organizationId,
    services: SAFE_IMAGE_SERVICES,
    fetchImpl,
  });
}
