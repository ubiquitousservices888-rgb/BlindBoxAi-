...
function documentedSaleCount(figure, transactionEvidence) {
  ...
  const explicitCount = transactionEvidence.match(/\b(\d+)\s+(?:firm\s+|completed\s+|reviewed\s+)?(?:US\s+)?(?:sales|transactions|completed sales)\b/i);
  if (explicitCount) return Number(explicitCount[1]);

  const singular = /\b(?:1\s+)?(?:US\s+)?(?:sale|completed transaction|completed sale)\b/i.test(transactionEvidence);
  return singular ? 1 : 0;
}
