...
function rowValue(row, aliases) {
  ...
  for (const [key, value] of Object.entries(row)) {
    if (normalizedAliases.includes(normalizeMetricKey(key))) return value;
  }
  return undefined;
}
...
export function summarizeEpnPerformancePayload(payload, {
  ...
  for (const row of records) {
    ...
    const status = rowValue(row, ["status", "transactionStatus", "transaction_status"]);
    if (!allowedStatus(status)) continue;

    const rowClicks = finiteNumber(
      rowValue(row, ["Clicks", "clicks", "clickCount", "click_count", "networkClicks"]),
    );
    const rowOrders = finiteNumber(
      rowValue(row, ["Transactions", "transactions", "Orders", "orders", "transactionCount", "salesCount"]),
    );
    const rowEarnings = finiteNumber(
      rowValue(row, ["Earnings", "earnings", "partnerEarnings", "partner earnings", "estimatedEarnings"]),
    );

    networkClicks += Math.max(0, rowClicks);
    orders += Math.max(0, rowOrders);
