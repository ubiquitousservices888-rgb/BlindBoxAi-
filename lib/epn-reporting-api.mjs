...
function rowDate(row) {
  const value = row?.Date ?? row?.date ?? row?.ReportDate ?? row?.reportDate ?? row?.Day ?? row?.day ?? row?.StartDate ?? row?.startDate;
...
export function summarizeEpnPerformancePayload(payload, {
  ...
  for (const row of records) {
    ...
    const clicks = finiteNumber(row.Clicks ?? row.clicks);
    const rowOrders = finiteNumber(row.Transactions ?? row.transactions ?? row.Orders ?? row.orders);
    const rowEarnings = finiteNumber(row.Earnings ?? row.earnings);
  ...
  const dailyRows = [...daily.values()]
    .map((item) => ({ ...item, earnings: roundMoney(item.earnings), epc: item.clicks > 0 ? roundEpc(item.earnings / item.clicks) : null }))
