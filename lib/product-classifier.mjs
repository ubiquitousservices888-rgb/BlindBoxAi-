...
export function classifyProduct(product) {
  ...
  if (accessoryMatches.length && !brandMatches.length) {
    const onlyGenericBlindBoxSignal = figureKeywordMatches.length > 0 &&
      figureKeywordMatches.every((signal) => signal === "blind box" || signal === "blind-box");
    if (!figureKeywordMatches.length || onlyGenericBlindBoxSignal) {
      return {
        type: "accessory",
