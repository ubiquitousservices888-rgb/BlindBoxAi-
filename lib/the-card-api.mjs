...
function titleHasPhrase(title, term) {
  ...
  // Marketplace titles often differ only by a trailing plural (Celebration/Celebrations).
  // Keep this narrow: single alphabetic words of five or more characters only.
  if (/^[a-z]{5,}$/.test(needle)) {
    const alternate = needle.endsWith("s") ? needle.slice(0, -1) : `${needle}s`;
    return alternate.length >= 5 && haystack.includes(` ${alternate} `);
  }
  return false;
}
...
function printRunMatches(title, printRunMax) {
  ...
  const denominator = String(printRunMax);
  return new RegExp(`(?:/\\s*${denominator}\\b|\\bof\\s+${denominator}\\b)`, "i").test(String(title ?? ""));
}

function editionMatches(title, editionValue) {
  const edition = clean(editionValue);
  if (!edition) return true;
  if (/unresolved/i.test(edition)) return false;
  // Serial numbers are already constrained by printRunMax; marketplace titles
  // frequently drop leading zeroes from the numerator.
  const serialParts = edition.split("/").map((part) => part.trim());
  if (serialParts.length === 2 && serialParts.every((part) => part && [...part].every((ch) => ch >= "0" && ch <= "9"))) return true;
  return titleHasPhrase(title, edition);
}
...
export function cardApiExactTargetMatches(title, target = {}) {
  ...
  if (!editionMatches(title, target?.identity?.edition)) return false;
  const condition = clean(target?.identity?.condition).toLowerCase();
  if (condition === "raw" && GRADED_TITLE_PATTERN.test(String(title ?? ""))) return false;
  if (condition === "graded" && !GRADED_TITLE_PATTERN.test(String(title ?? ""))) return false;
  return true;
}
...
export async function fetchCardApiSales(target, options = {}) {
  ...
  const query = clean(target?.providerQuery || target?.query);
...
if (response.status >= 300 && response.status < 400) {
  ...
  if (!location) {
    return {
      ...
      records: [],
      error: "provider redirect missing location",
      providerQuery: query,
    };
  }
  ...
  if (
    ...
    !allowedHosts.has(redirectedUrl.hostname)
  ) {
    return {
      status: "unsafe_redirect",
      records: [],
      error: "provider redirected outside allowed host",
      providerQuery: query,
    };
  }

  response = await fetchImpl(redirectedUrl, {
    signal: controller.signal,
    redirect: "error",
    headers: { "x-market-api-key": apiKey, accept: "application/json" },
  });
    ...
    if (!response.ok) return { status: `http_${response.status}`, records: [], error: "provider request failed", providerQuery: query };
