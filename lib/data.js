...
export function seriesVerified(s) {
  ...
  return priced.length > 0 && priced.every(f =>
    !f.needsReview &&
    Array.isArray(f.saleEvidence) &&
    new Set(f.saleEvidence.map(e => e.source)).size >= 2
  );
}
...
export function ebayOutboundPath(seriesSlug, figure, kind, attribution = {}) {
  ...
  const source = isValidSource(attribution.source) ? attribution.source : "none";
  const vertical = ["bb", "sc", "tc"].includes(attribution.vertical)
