...
function safeEbayError(body) {
  ...
  const domain = String(first.domain ?? "").trim().slice(0, 40);
  const category = String(first.category ?? "").trim().slice(0, 40);
  const message = String(first.message ?? "").replace(/\s+/g, " ").trim().slice(0, 180);
  if (!errorId && !domain && !category && !message) return null;
  return { errorId, domain, category, message };
}

async function readSellerCollection(url, accessToken, collectionField) {
