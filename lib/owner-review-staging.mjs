...
function validateVideoUrl(value) {
  ...
  if (!url.hostname.endsWith(".public.blob.vercel-storage.com")) throw new Error("videoUrl must use the approved Vercel Blob media host");
...
export function researchRunIdForVideo(videoUrl) {
  const safeVideoUrl = validateVideoUrl(videoUrl);
  const digest = createHash("sha256").update(safeVideoUrl).digest("hex").slice(0, 16);
  return `rv-${digest}`;
}
...
export async function stageOwnerReviewedVideo({
  ...
  const safeVideoUrl = validateVideoUrl(videoUrl);
  const safeVideoTitle = requirePublicVideoTitle(title, { label: "title", maxLength: 100 });
  ...
  const researchRunId = researchRunIdForVideo(safeVideoUrl);
