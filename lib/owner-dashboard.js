...
export async function getOwnerDashboardSnapshot({ ifNoneMatch = "", lookbackDays } = {}) {
  ...
  const [clickBlobs, notificationBlobs, funnelBlobs] = await Promise.all([
  ...
  const [epnClicks, notifications, funnelEvents] = await Promise.all([
  ...
  const allFunnelEvents = [...funnelEvents, ...legacyOnlyClicks];
