...
import { AMAZON_ASSOCIATE_TAG, amazonOutboundPath, buildAmazonSearchUrl } from "./amazon-associates.mjs";
...
export function validateAffiliatePathForProduct(product) {
  ...
  if (route.path === "amazon") {
    ...
    if (!outbound.startsWith("/api/out/amazon?")) {
      throw new Error("Amazon accessories must route through /api/out/amazon");
    }
    audit.push(`amazon_offer:${offerId}`);
    audit.push(`amazon_outbound:${outbound}`);
...
export function generateVideoScript(product, date = new Date()) {
  ...
  const audienceQuestion = product.videoMode === AUDIENCE_PRICE_MODE
    ...
    : null;
  const narration = [`Collector check: ${product.name}.`, ...facts, audienceQuestion, `See the verified details: ${product.productUrl}`].filter(Boolean).join(" ");
  ...
  return { title: product.name, narration, caption, facts, productUrl: product.productUrl, videoMode: product.videoMode ?? "VERIFIED_FACTS" };
}
