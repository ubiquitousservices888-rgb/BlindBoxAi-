...
export function validateAffiliatePathForProduct(product) {
  ...
  if (route.path === "ebay") {
    const url = buildEbaySearchUrl({ query: product?.name, kind: "active" });
    const epnAudit = auditEpnUrl(url, { kind: "active", requireTracking: false });
    if (!epnAudit.ok) {
      throw new Error(`eBay audit failed: ${epnAudit.reasons.join("; ")}`);
    }
    const status = classification.reason.includes("and accessory signals") ? "BOTH" : "EBAY";
