...
import { AMAZON_ASSOCIATE_TAG, buildAmazonSpecialLink } from "./amazon-associates.mjs";
import {
  CAPTION_LIMITS,
...
const TRACKED_FIGURE_BRANDS = new Set(["pop mart", "sonny angel", "smiski"]);
const FIGURE_TOKENS = [
  "blind box",
  "blind-box",
  "figure",
...
const AMBIGUOUS_TOKENS = ["bundle", "combo", "mixed", "assorted"];
...
export function pickAffiliateMonetizationPath(product = {}) {
  ...
  const text = [name, brand, category].filter(Boolean).join(" ");
  const isTrackedFigure = TRACKED_FIGURE_BRANDS.has(brand) || hasAnyToken(text, FIGURE_TOKENS);
  const isAccessory = hasAnyToken(text, ACCESSORY_TOKENS);
  const isExplicitlyMixed = hasAnyToken(text, AMBIGUOUS_TOKENS);

  if (isTrackedFigure && isAccessory && isExplicitlyMixed) {
    return {
  ...
  if (isAccessory) {
    return {
      ...
      monetizationPath: "amazon-associates",
      validation: "AMAZON",
      destinationUrl: buildAmazonSpecialLink(product?.name || product?.query || "collector accessories"),
    };
  }
...
export function assertAffiliatePathValidation(script) {
  const affiliate = script?.affiliatePath;
  if (!affiliate || typeof affiliate !== "object") throw new Error("Affiliate path is required");

  if (affiliate.monetizationPath === "existing-ebay-epn") {
    if (affiliate.validation !== "EPN") throw new Error("Figure affiliate path must validate through EPN");
    return true;
  }

  if (affiliate.monetizationPath === "amazon-associates") {
    if (affiliate.validation !== "AMAZON") throw new Error("Accessory affiliate path must validate through Amazon");
    const target = new URL(String(affiliate.destinationUrl ?? ""));
    if (target.protocol !== "https:" || target.hostname !== "www.amazon.com") {
      throw new Error("Amazon affiliate path must be a direct Amazon HTTPS URL");
    }
    if (target.searchParams.get("tag") !== AMAZON_ASSOCIATE_TAG) {
      throw new Error("Amazon affiliate tag is invalid");
    }
    if (/\/api\/out\/(?:ebay|offer|ebay-live)/i.test(target.pathname)) {
      throw new Error("Amazon affiliate path must not use the EPN redirect system");
    }
    return true;
  }

  if (affiliate.monetizationPath === "NONE") return true;
  throw new Error("Affiliate path is not approved");
}
...
export function generateVideoScript(product, date = new Date()) {
  ...
  const affiliatePath = pickAffiliateMonetizationPath(product);
  ...
  return { title: product.name, narration, caption, facts, productUrl: product.productUrl, affiliatePath };
}
...
export async function publishApproved(record, publishChannel, now = new Date()) {
  ...
  assertAffiliatePathValidation(record.script);
