...
import { AMAZON_ASSOCIATE_TAG, buildAmazonSearchUrl } from "./amazon-associates.mjs";
import { auditEpnUrl, buildEbaySearchUrl } from "./affiliate-policy.mjs";
import { classifyProduct } from "./product-classifier.mjs";
import { resolveAccessoryOfferId, routeProductToAffiliate } from "./monetization-router.mjs";
import {
  CAPTION_LIMITS,
...
export const AUDIENCE_PRICE_MODE = "AUDIENCE_PRICE_QUESTION";
export const AUDIENCE_PRICE_DISCLOSURE = "Viewer raw and graded answers are audience research only, not completed-sale evidence or verified market value.";
export const AMAZON_VIDEO_CTA = "https://blindboxai.com/shop/accessories";
...
export function validateAffiliatePathForProduct(product) {
  ...
  if (route.path === "ebay") {
    let campid = String(process.env.NEXT_PUBLIC_EPN_CAMPID || "").trim();
    if (!/^\d{7,12}$/.test(campid)) {
      throw new Error("NEXT_PUBLIC_EPN_CAMPID must be configured for eBay affiliate validation");
    }
    const url = new URL(buildEbaySearchUrl({
      query: product?.name,
      kind: "active",
      campid,
      customId: "video-pipeline-affiliate-check",
    }));
    const epnAudit = auditEpnUrl(url, { kind: "active", requireTracking: true });
    if (!epnAudit.ok) {
      throw new Error(`eBay audit failed: ${epnAudit.reasons.join("; ")}`);
    }
    const status = classification.signals?.hasFigure && classification.signals?.hasAccessory
      ? "BOTH"
      : "EBAY";
    audit.push(`ebay_url:${url}`);
    return { status, audit };
  }

  if (route.path === "amazon") {
    const offerId = resolveAccessoryOfferId(product);
    if (!offerId) throw new Error("Amazon accessory offer resolution failed");
    const target = new URL(buildAmazonSearchUrl(offerId));
    if (target.searchParams.get("tag") !== AMAZON_ASSOCIATE_TAG) {
      throw new Error("Amazon associate tag mismatch");
    }
    const cta = AMAZON_VIDEO_CTA;
    if (
      cta !== "https://blindboxai.com/shop/accessories" ||
      cta.includes("/api/out/amazon") ||
      /amazon\.com/i.test(cta)
    ) {
      throw new Error("Amazon video CTA must use the BlindBoxAI accessories landing page");
    }
    audit.push(`amazon_offer:${offerId}`);
    audit.push(`amazon_cta:${cta}`);
    return { status: "AMAZON", audit };
  }

  return { status: "NONE", audit };
}
...
export function validateVerifiedProduct(product, now = new Date()) {
  ...
  for (const source of product.sources) {
    ...
    if (!source.id || source.status !== "verified" || !httpsUrl(source.url) ||
        !isoDate(source.checkedAt) || !Number.isFinite(checkedAtMs)) {
      throw new Error(`Source ${source.id ?? "unknown"} is not verified`);
    }
  ...
  if (product.videoMode === AUDIENCE_PRICE_MODE) {
    const publicClaims = product.claims.map((claim) => claim.text).join(" ");
    if (/\$\s*\d|\b(?:worth|steal|premium|invest(?:or|ment|ing)|profit|market price|market value|sold for)\b/i.test(publicClaims)) {
      throw new Error("Audience price-question mode cannot contain a price or valuation claim");
    }
  }
  try {
    validateAffiliatePathForProduct(product);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`Affiliate routing validation failed: ${message}`, { cause: error });
  }
  return product;
}
...
export function generateVideoScript(product, date = new Date()) {
  ...
  const audienceQuestion = product.videoMode === AUDIENCE_PRICE_MODE
    ? `No matching completed sales were verified for this exact card. What would you pay for it raw? What would you pay for it graded? ${AUDIENCE_PRICE_DISCLOSURE}`
    : null;
  const route = routeProductToAffiliate(product);
  const productUrl = route.path === "amazon" ? AMAZON_VIDEO_CTA : product.productUrl;
  if (
    route.path === "amazon" &&
    (productUrl !== AMAZON_VIDEO_CTA || productUrl.includes("/api/out/amazon") || /amazon\.com/i.test(productUrl))
  ) {
    throw new Error("Amazon video CTA must use the BlindBoxAI accessories landing page");
  }
  const narration = [`Collector check: ${product.name}.`, ...facts, audienceQuestion, `See the verified details: ${productUrl}`].filter(Boolean).join(" ");
  const caption = `${product.name}\n\n${facts.join("\n")}${audienceQuestion ? `\n\n${audienceQuestion}` : ""}\n\n${productUrl}\n\n${DISCLOSURE}`;
  if (!caption.includes(DISCLOSURE) || !caption.includes(productUrl)) throw new Error("BlindBoxAI CTA and EPN disclosure are required");
  return { title: product.name, narration, caption, facts, productUrl, videoMode: product.videoMode ?? "VERIFIED_FACTS" };
}
...
export function assertPublishableState(record) {
  if (![STATES.APPROVED, STATES.PARTIAL].includes(record?.state)) {
...
export async function renderCreatomate({ apiKey, templateId, record, fetchImpl = fetch, pollMs = 3000, maxPolls = 60 }) {
  ...
  let render = Array.isArray(created) ? created[0] : created;
  if (!render?.id || !render?.status) throw new Error("Creatomate render request returned an invalid response");
