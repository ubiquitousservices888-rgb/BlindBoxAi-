...
export function validateAffiliatePathForProduct(product, { allowPlaceholderTracking = false } = {}) {
  ...
  if (route.path === "ebay") {
    let campid = String(process.env.EPN_CAMPID || process.env.NEXT_PUBLIC_EPN_CAMPID || "").trim();
    if (!/^\d{7,12}$/.test(campid)) {
      if (!allowPlaceholderTracking) {
        throw new Error("EPN_CAMPID must be configured for eBay affiliate validation");
      }
      campid = "1234567";
      audit.push("ebay_tracking:validation-placeholder-campid");
    }
...
export function validateVerifiedProduct(product, now = new Date()) {
  ...
  try {
    validateAffiliatePathForProduct(product, { allowPlaceholderTracking: true });
  } catch (error) {
    throw new Error(`Affiliate routing validation failed: ${error.message}`);
  }
  return product;
}
