...
export function generateVideoScript(product, date = new Date()) {
  ...
  if (!caption.includes(DISCLOSURE)) throw new Error("EPN disclosure is required");
...
export function approve(record, now = new Date()) {
  ...
  if (!record.script?.caption?.includes(DISCLOSURE)) throw new Error("Cannot approve without EPN disclosure");
...
export async function publishApproved(record, publishChannel, now = new Date()) {
  ...
  if (!record.render?.videoUrl || !record.script?.caption?.includes(DISCLOSURE)) throw new Error("Hosted video and EPN disclosure are required");
...
export function createBufferPublisher({ token, channelProfiles, fetchImpl = fetch }) {
  if (!token) throw new Error("BUFFER_API_TOKEN is required");
  return async ({ channel, videoUrl, caption, idempotencyKey }) => {
    const profileId = channelProfiles[channel];
    if (!profileId) throw new Error(`Missing Buffer profile for ${channel}`);
    const response = await fetchImpl("https://api.bufferapp.com/1/updates/create.json", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "Idempotency-Key": idempotencyKey }, body: JSON.stringify({ profile_ids: [profileId], text: caption, media: { video: videoUrl }, now: true }) });
    if (!response.ok) throw new Error(`Buffer ${channel} publish failed: ${response.status}`);
    const body = await response.json();
    return { id: body.updates?.[0]?.id ?? body.id ?? idempotencyKey };
  };
}
