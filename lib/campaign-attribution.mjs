...
export function resolveRequestCampaign({ campaign, source, referer } = {}) {
  const explicitCampaign = normalizeCampaignId(campaign);
  if (explicitCampaign) {
    return { campaignId: explicitCampaign, source: normalizeSource(source) };
  }

  try {
    const ref = new URL(String(referer || ""));
    if (ref.protocol !== "https:" || !BLINDBOX_HOSTS.has(ref.hostname)) {
      return { campaignId: "", source: normalizeSource(source) };
    }
    const refCampaign = normalizeCampaignId(ref.searchParams.get("campaign"));
    if (!refCampaign) return { campaignId: "", source: normalizeSource(source) };
    return {
      campaignId: refCampaign,
      source: normalizeSource(ref.searchParams.get("source") || source),
    };
  } catch {
    return { campaignId: "", source: normalizeSource(source) };
  }
