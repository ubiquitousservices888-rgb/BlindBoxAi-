...
function reviewedCatalogRecord(series, now) {
  const eligibility = evaluateAffiliateEligibility(series, { now });
  const reviewedFigures = eligibility.verifiedMarketRecords.map((record) => ({
    name: record.figure,
    rarity: record.rarity,
    observedUsdRange: [record.resaleLowUSD, record.resaleHighUSD],
    evidence: record.transactionEvidence,
    completedSaleCount: record.completedSaleCount,
    latestSaleAt: record.latestSaleAt,
  ...
  return {
    ...
    reviewedFigures,
    pendingFigureCount: Math.max(0, (series.figures ?? []).length - reviewedFigures.length),
  };
}

export function buildCatalogSnapshot(seriesDirectory = path.join(process.cwd(), "data", "series"), { now = new Date() } = {}) {
  ...
  return fs.readdirSync(seriesDirectory)
    ...
    .map((series) => reviewedCatalogRecord(series, now))
...
- Treat the BlindBoxAI catalog below as local reviewed evidence only where reviewStatus is "reviewed". Every included price has already passed BlindBoxAI's shared requirement of at least two documented completed sales. Pending records are deliberately omitted.
- Public price freshness uses a ${PUBLIC_PRICE_FRESHNESS_DAYS}-day window from the latest documented completed sale. If freshnessStatus is "dated", describe the range as dated historical evidence, not a current price. If freshnessStatus is "unknown", say the sale-date freshness is unknown.
- Always state completedSaleCount and latestSaleAt when discussing a local catalog price if they are available.
...
export async function askMrKnowItAll(question, options = {}) {
  ...
  const catalog = options.catalog ?? buildCatalogSnapshot(undefined, { now });
