...
const BLOCKED_FINANCIAL_LANGUAGE = [
  ...
  /\b(?:price[-\s]+prediction|predicted price|price forecast|price target)\b/i,
  /\bbuy now before\b/i,
...
function safeEvidence(series) {
  ...
  return figures.filter((figure) => {
    return figure?.needsReview === false && String(figure?.evidence ?? "").trim().length > 0;
  });
}
...
function financialScanText(candidate, publicText) {
  let text = publicText;
  for (const name of [candidate?.source?.seriesName, candidate?.source?.figureName]) {
    const normalized = String(name ?? "").trim();
    if (!normalized) continue;
    text = text.replace(new RegExp(escapeRegExp(normalized), "gi"), "[verified product name]");
  }
  return text;
}
...
export function assertNarrativeSafety(candidate) {
  ...
  const financialText = financialScanText(candidate, publicText);
  for (const rule of BLOCKED_FINANCIAL_LANGUAGE) {
    if (rule.test(financialText)) throw new Error(`Narrative public copy contains blocked financial language: ${rule}`);
  }
