...
function validPlatformPostUrl(service, url) {
  ...
  if (service === "youtube") {
    if (hostMatches(host, "youtu.be")) return /^\/[A-Za-z0-9_-]{6,}(?:\/)?$/.test(url.pathname);
  ...
  if (service === "tiktok") {
    return hostMatches(host, "tiktok.com") && /^\/@[^/]+\/video\/\d+(?:\/)?$/.test(url.pathname);
  }
  return false;
}
...
export function assertVerifiedPublicPost({ channel, externalLink, text, expectedCaption }) {
  ...
  if (!validPlatformPostUrl(service, url)) {
