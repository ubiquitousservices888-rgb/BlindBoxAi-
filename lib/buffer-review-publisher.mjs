...
function validPlatformPostUrl(service, url) {
  ...
  if (service === "twitter") {
    return /^\/(?:i\/web\/|[^/]+\/)status\/\d+(?:\/)?$/.test(url.pathname);
  }
  return false;
}
...
export function createReviewBufferPublisher({ token, organizationId, fetchImpl = fetch }) {
  ...
  return async ({ channel, videoUrl, caption, title, youtubeCategoryId = YOUTUBE_SPORTS_CATEGORY_ID }) => {
    ...
    const input = {
      ...
      ...(channel === "youtube"
        ? { metadata: { youtube: { title: youtubeTitle(title), categoryId: String(youtubeCategoryId) } } }
        : {}),
    };

    const data = await bufferGraphQL(
      token,
      `mutation CreateReviewVideo($input: CreatePostInput!) {
