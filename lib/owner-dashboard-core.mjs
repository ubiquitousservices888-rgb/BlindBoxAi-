...
export const MAX_DASHBOARD_LOOKBACK_DAYS = 30;
export const CLICK_DISPLAY_LIMIT = 24;
export const NOTIFICATION_DISPLAY_LIMIT = 12;
export const FUNNEL_DISPLAY_LIMIT = 40;
...
export function normalizeLookbackDays(value, fallback = DASHBOARD_LOOKBACK_DAYS) {
  const days = Number(value);
  if (!Number.isInteger(days)) return fallback;
  return Math.min(MAX_DASHBOARD_LOOKBACK_DAYS, Math.max(1, days));
}
...
export async function listAllBlobPages(listPage, { prefix, limit = LIST_PAGE_SIZE }) {
  ...
  let cursor;
  while (true) {
    const page = await listPage({ prefix, limit, ...(cursor ? { cursor } : {}) });
    if (!page || !Array.isArray(page.blobs)) throw new Error(`Invalid Blob list response for ${prefix}.`);
    blobs.push(...page.blobs);
    if (!page.hasMore) return blobs;
    if (!page.cursor || seenCursors.has(page.cursor)) throw new Error(`Blob pagination did not advance for ${prefix}.`);
    seenCursors.add(page.cursor);
    cursor = page.cursor;
  }
}

export async function listRecentBlobMetadata({ listPage, prefix, now = new Date(), days = DASHBOARD_LOOKBACK_DAYS, pageSize = LIST_PAGE_SIZE }) {
  const dates = recentUtcDateStrings(now, days);
  const pages = await Promise.all(dates.map((date) => listAllBlobPages(listPage, { prefix: `${prefix}${date}/`, limit: pageSize })));
  const unique = new Map();
  for (const blob of pages.flat()) if (blob?.pathname) unique.set(blob.pathname, blob);
...
export function countBlobsInWindow(blobs, { from, through }) {
  ...
  if (!Number.isFinite(fromTime) || !Number.isFinite(throughTime)) throw new Error("Invalid Blob count window.");
...
function blobIdentity(blob) {
  return [String(blob.pathname || ""), String(blob.etag || ""), Number(blob.size || 0), dateValue(blob.uploadedAt)];
}
...
export function buildDashboardEtag({
  ...
  const payload = JSON.stringify({
    ...
    funnelBlobs: funnelBlobs.map(blobIdentity),
    evidenceBlobs: evidenceBlobs.map(blobIdentity),
  });
...
export function requestEtagMatches(headerValue, etag) {
  ...
  return String(headerValue || "").split(",").map((value) => value.trim()).some((value) => value === "*" || value.replace(/^W\//, "") === expected);
}
