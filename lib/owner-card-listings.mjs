...
const LISTINGS_FILE = new URL("../data/know-it-all/owner-ebay-listings.json", import.meta.url);
const TARGETS_FILE = new URL("../data/know-it-all/sports-card-research-targets.json", import.meta.url);

function readJson(url) {
  return JSON.parse(fs.readFileSync(url, "utf8"));
}

const listingDocument = readJson(LISTINGS_FILE);
const targetDocument = readJson(TARGETS_FILE);
